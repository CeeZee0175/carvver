...
create unique INDEX IF not exists customer_checkout_sessions_paymongo_checkout_id_key on public.customer_checkout_sessions using btree (paymongo_checkout_id) TABLESPACE pg_default
where
  (paymongo_checkout_id is not null);

create unique INDEX IF not exists customer_checkout_sessions_payment_intent_id_key on public.customer_checkout_sessions using btree (paymongo_payment_intent_id) TABLESPACE pg_default
where
  (paymongo_payment_intent_id is not null);

create index IF not exists customer_checkout_sessions_user_status_idx on public.customer_checkout_sessions using btree (user_id, status, created_at desc) TABLESPACE pg_default;

create index IF not exists customer_checkout_sessions_snapshot_idx on public.customer_checkout_sessions using btree (
  user_id,
  cart_snapshot_hash,
